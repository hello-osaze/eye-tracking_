...
class IITBHGCDataset(ETDataset):
    ...
    def convert_examples_to_features(
        ...
        features, labels = super().convert_examples_to_features(text_data=text_data)

        gold_labels = []
        annotator_labels = []
        for grouped_data_key in self.ordered_key_list:
            trial = self.grouped_ia_data.get_group(grouped_data_key)
            trial_row = trial.iloc[0]
            gold_labels.append(
                int(
                    trial_row[Fields.GOLD_LABEL]
                    if Fields.GOLD_LABEL in trial_row.index
                    else trial_row[self.target_column]
                )
            )
            annotator_labels.append(
                int(
                    trial_row[Fields.ANNOTATOR_LABEL]
                    if Fields.ANNOTATOR_LABEL in trial_row.index
                    else trial_row[self.target_column]
                )
            )

        features['gold_labels'] = torch.tensor(gold_labels, dtype=torch.long)
        features['annotator_labels'] = torch.tensor(annotator_labels, dtype=torch.long)
